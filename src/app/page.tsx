...
// Bank picker — inline popup shown when store button pressed
...
function BankPicker({ source, onSelect, onCancel, banks }: BankPickerProps) {
  return (
    <div style={{
      position:'absolute', bottom:'calc(100% + 4px)', left:0, right:0,
      background:'var(--cream-light)', border:'1px solid var(--border)',
      borderBottom:'none', zIndex:100, padding:'12px 16px',
        ...
        {banks.map((bank, i) => (
          <button
            ...
            style={{
              ...
              background: bank.state==='EMPTY' ? BANK_PICKER_COLORS[i % BANK_PICKER_COLORS.length] : 'var(--cream-dark)',
              border:`2px solid ${bank.state==='EMPTY' ? BANK_PICKER_COLORS[i % BANK_PICKER_COLORS.length] : 'var(--border)'}`,
              color: bank.state==='EMPTY' ? 'white' : 'var(--light)',
              clipPath:'polygon(5px 0%, 100% 0%, calc(100% - 5px) 100%, 0% 100%)',
              opacity: bank.state==='EMPTY' ? 1 : 0.6,
            }}
          >
            {bank.name}
            {bank.state !== 'EMPTY' && (
              <span style={{ display:'block', fontSize:'9px', letterSpacing:'1px', color:'var(--light)', marginTop:'2px' }}>overwrite</span>
            )}
          </button>
        ))}
        <button
          onClick={onCancel}
          style={{ fontFamily:'Space Mono, monospace', fontSize:'10px', padding:'8px 12px', background:'transparent', border:'1px solid var(--border)', color:'var(--light)', cursor:'pointer', marginLeft:'auto', alignSelf:'center' }}
        >cancel</button>
      </div>
    </div>
  );
...
function ToneControls() {
  ...
  const smBtn = (active:boolean,ac='var(--blue)',ab='var(--blue-dark)') => ({ fontFamily:'Space Mono, monospace' as const,fontSize:'11px',padding:'4px 8px',cursor:'pointer' as const,background:active?ac:'var(--cream-light)',border:`1px solid ${active?ab:'var(--border)'}`,color:active?'white':'var(--mid)' });
  ...
  return (
    <div style={{ display:'flex',flexDirection:'column',gap:'8px',width:'220px' }}>
      <div style={{ display:'flex',gap:'3px',flexWrap:'wrap' }}>
        {KEYS.map(k=><button key={k} onClick={()=>setRootNote(k)} style={{...smBtn(rootNote===k),minWidth:'28px',textAlign:'center' as const,padding:'4px 5px'}}>{k}</button>)}
      </div>
      <div style={{ display:'flex',alignItems:'center',gap:'6px' }}>
        {rowLabel('OCT')}
        <div style={{ display:'flex',gap:'4px' }}>{[1,2,3,4].map(o=><button key={o} onClick={()=>setOctave(o)} style={{...smBtn(octave===o),padding:'5px 12px'}}>{o}</button>)}</div>
      </div>
      <div style={{ display:'flex',alignItems:'center',gap:'6px' }}>
        {rowLabel('MODE')}
...
function SignalSection({ inputLabel,isField=false,chain,banks,onStoreToBank }: {
  ...
  return (
    <div style={{ display:'flex',gap:'24px',alignItems:'flex-start' }}>
      <div style={{ display:'flex',flexDirection:'column',alignItems:'center',gap:'10px',flexShrink:0,width:'220px' }}>
        ...
        />
        {!isField?<ToneControls/>:<FieldControls recRunning={recRunning} recSecs={recSecs} onRec={startRec} onStop={stopRec}/>}
      </div>
      ...
      <div style={{ width:'420px',display:'flex',flexDirection:'column',position:'relative' }}>
        ...
        {/* Store button — same style for both Tone and Field */}
        <div style={{ position:'relative' }}>
          <button
            onClick={()=>setShowPicker(p=>!p)}
            style={{
              width:'100%',padding:'12px 16px',
              background: showPicker ? 'var(--blue)' : 'transparent',
              border:`1px solid var(--blue)`, borderTop:'none',
              cursor:'pointer',display:'flex',alignItems:'center',gap:'8px',
              fontFamily:'Rajdhani, sans-serif',fontWeight:600,
              fontSize:'13px',letterSpacing:'2px',
              color: showPicker ? 'white' : 'var(--blue)',
...
export default function Home() {
  ...
  // Banks state lifted up so signal sections can read bank names/states
      ...
      <div style={{ flex:1,paddingBottom:'24px' }}>
        ...
        <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0',padding:'0 24px' }}>
          <div style={{ borderRight:'1px solid var(--border)',paddingRight:'28px' }}>
            ...
            <SignalSection inputLabel="Input 1 · Tone" chain={toneChain} banks={bankList} onStoreToBank={handleStoreToBank} />
          </div>
        ...
        <Banks
          storeFnRef={storeFnRef}
          onBankListChange={setBankList}
        />
      </div>
      <FooterBar />
    </div>
